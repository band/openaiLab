...
def gpt_chat_completion(prompt, engine='gpt-3.5-turbo', temp=0.6, top_p=1.0, tokens=1800, freq_pen=0.25, pres_pen=0.0, stop=['<<END>>']):
    max_retry = 5
    retry = 0
    while True:
        try:
...
if __name__ == '__main__':
    ...
    for chunk in chunks:
        count = count + 1
        prompt = open_file('prompt-cc.txt').replace('<<CHUNK>>', chunk)
